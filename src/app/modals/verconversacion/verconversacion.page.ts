...
export class VerconversacionPage implements OnInit {
  ...
  async Traermismensajesdechatabierto(){
    var databeoboxtraermensajesdeconversacion = {
      nombre_solicitud: 'beoboxtraermensajesdeconversacion',
      id_conversacion:this.dataparaelmodal.id
      }
      this.varios.variasfunciones(databeoboxtraermensajesdeconversacion).subscribe(async( res: any ) =>{
      console.log('respuesta de beoboxtraermensajesdeconversacion', res);
  ...
  marcarresuelto(){
    var databeoboxcerrarconversacion = {
      nombre_solicitud: 'beoboxcerrarconversacion',
      id:this.dataparaelmodal.id
    }
    this.varios.variasfunciones(databeoboxcerrarconversacion).subscribe(async( res: any ) =>{
      console.log('respuesta de beoboxcerrarconversacion', res);
      if(res){
        this.Traermismensajesdechatabierto();
        this.enviomensaje();
  ...
  EnviarMensajeDeChat(){
    var dataguardarmensajedeusuario = {
      nombre_solicitud: 'beoboxguardarmensajedeusuario',
